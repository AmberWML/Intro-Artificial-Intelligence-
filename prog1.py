...
def compare_searchers(problems, header,
    ...
    for p in problems:
        for s in searchers:
            arr.append(s(p).path_cost)
            # print(s(p).path_cost)
    # print(lst)
    mean1= statistics.mean(arr[0:5])
    mean2 = statistics.mean(arr[5:10])
    mean3 = statistics.mean(arr[10: ])

    s1= statistics.stdev(arr[0:5])
    s2= statistics.stdev(arr[5:10])
    s3= statistics.stdev(arr[10: ])
    print(mean1)
    print(mean2)
    print(mean3)
...
def compare_graph_searchers():
    ...
    compare_searchers(problems=[GraphProblem('Arad', 'Bucharest', romania_map),
                      ...
                      header=['Searcher', 'romania_map(Arad, Bucharest)',
                              'romania_map(Oradea, Neamt)', 'romania_map(Eforie, Timisoara)'])
